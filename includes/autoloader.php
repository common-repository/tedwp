 ...
 * ted autoloader.
 *
 * ted autoloader handler class is responsible for loading the different
...
class Autoloader
{
     ...
     * Classes map.
     *
     * Maps ted classes to file names.
     *
     * @since 1.6.0
     * @access private
     * @static
     *
     * @var array Classes used by ted.
     */
    ...
    private static function load_class($relative_class_name)
    {
        ...
        if (isset(self::$classes_map[$relative_class_name])) {
            $filename = TED_PATH . '/' . self::$classes_map[$relative_class_name];
        } else {
            $filename = strtolower(
                preg_replace(
                    ['/([a-z])([A-Z])/', '/_/', '/\\\/'],
            ...
            $filename = TED_PATH . '/' . $filename . '.php';
        }
